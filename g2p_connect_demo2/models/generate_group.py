...
class OpenG2PGenerateData(models.Model):
    ...
    def _generate_individual_data(
        ...
        date_start_pregnancy = birthdate + relativedelta(years=18)
        date_end_pregnancy = datetime.datetime.now()

        date_pregnancy = None
        if not date_start_pregnancy > datetime.datetime.now() and sex == "Female":
            date_pregnancy = fake.date_between_dates(
                date_start=date_start_pregnancy, date_end=date_end_pregnancy
            ).isoformat()
